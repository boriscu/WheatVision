...
from external.sam2_repo.sam2 import build_sam2_video_predictor
...
class VideoTrackingService:
    ...
    def __init__(
        self, model_config_name: str, checkpoint_path: str, device: str = "cuda"
    ):
        """
    ...
    def _ensure_predictor(self):
        ...
        if self.predictor is None:
            ...
            self.predictor = build_sam2_video_predictor(
                self.model_config_name, self.checkpoint_path, device=self.device
            )
    ...
    def add_click(
        self, frame_idx: int, points: np.ndarray, labels: np.ndarray, obj_id: int
    ):
        """
